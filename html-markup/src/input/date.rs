...
impl NativeComponent for InputDate {
    fn render(self, ctx: NativeContext) -> NativeNode {
        let node = NativeNode::new("input",ctx)
            ...
            .set_if("min", self.min.map(|d| format!("{}{:02}{:02}", d.year(), d.month(), d.day())))
            .set_if("max", self.max.map(|d| format!("{}{:02}{:02}", d.year(), d.month(), d.day())))
            .set_if("value", self.value.map(|d| format!("{}{:02}{:02}", d.year(), d.month(), d.day())))
            ;
