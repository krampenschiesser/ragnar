...
use crate::state_persistence::{SingleState, StoredMessage};
use crate::{EventExt, StateExt};
use ragnar_lib::{App, AppComponent, Runtime};
    ...
    > SessionActor<C, State, Msg>
{
    ...
    pub fn handle(mut self) -> anyhow::Result<()> {
        ...
        loop {
            let (cmd, response) = self.receiver.recv()?;
            match cmd {
