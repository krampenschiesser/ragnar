...
use crate::runtime::diff::operations::{
    AddNode, DiffOperation, ParentPosition, RemoveAttribute, RemoveListener, RemoveNode,
    SetAttribute, SetListener, SwapNode, SwapNodeId,
};
...
#[derive(Error, Debug)]
pub enum DiffError {
    #[error("handler already used callback={0:?} node={1:?}")]
    HandlerAlreadyUsed(CallbackId, NodeId),
    #[error("Non native node={0:?}")]
    NonNative(NodeId),
    #[error("Old node not found. node={0:?}")]
    OldNodeNotFound(NodeId),
    #[error("New node not found. node={0:?}")]
    NewNodeNotFound(NodeId),
    #[error("Old node callback not found. callback={0:?}")]
    OldCallbackNotFound(CallbackId),
    #[error("New node callback not found. callback={0:?}")]
...
impl<'a> CompleteDiff<'a> {
    ...
    fn diff_nodes(
        ...
        if original.get_id() == new.get_id() {
            ...
            diff_ops.push(DiffOperation::SwapNode(SwapNode::new(
                *original.get_id(),
                new,
                parent,
            )));
            ...
            diff_ops.push(DiffOperation::SwapNodeId(SwapNodeId {
                original: *original.get_id(),
                new: *new.get_id(),
            }));
    ...
    fn diff_attributes(
        ...
        let all_keys = original
            .get_attributes()
            .keys()
            .chain(new.get_attributes().keys());
        for key in all_keys {
            let original_value = original.get_attributes().get(key);
            let new_value = new.get_attributes().get(key);
            if original_value != new_value {
                if let Some(new_value) = new_value {
                    diff_ops.push(DiffOperation::SetAttribute(SetAttribute {
                        node_id: *new.get_id(),
                        attribute_name: key.clone(),
                        attribute_value: new_value.clone(),
                    }));
                } else {
                    diff_ops.push(DiffOperation::RemoveAttribute(RemoveAttribute {
                        node_id: *new.get_id(),
                        attribute_name: key.clone(),
                    }));
                }
        ...
        loop {
            ...
            if let Some(original_child) = original_child {
                if let Some(new_child) = new_child {
                    ...
                    diff_ops.push(DiffOperation::RemoveNode(RemoveNode {
                        node_id: *original_child.get_id(),
                    }))
                }
    ...
    fn diff_callbacks(
        ...
        original_callbacks
            .iter()
            .filter(|n| !new_callback_names.contains(&&n.native_name))
            .for_each(|n| {
                diff_ops.push(DiffOperation::RemoveListener(RemoveListener {
                    node_id: n.node_id,
                    callback_id: n.id,
                }))
            });
        new_callbacks
            .iter()
            .for_each(|n| diff_ops.push(DiffOperation::SetListener(SetListener::from(n))));
    }
}
